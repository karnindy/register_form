...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'add') {
        ...
        $stmt = $db->prepare("INSERT INTO mst_titles (name, status, display_order, create_by, create_date) VALUES (?, ?, ?, ?, NOW())");
        if ($stmt) {
        ...
        $stmt = $db->prepare("UPDATE mst_titles SET name=?, status=?, display_order=?, update_by=?, update_date=NOW() WHERE id=?");
        if ($stmt) {
    ...
    $stmt = $db->prepare("DELETE FROM mst_titles WHERE id = ?");
    if ($stmt) {
...
// Fetch all titles
$result = $db->query("SELECT * FROM mst_titles ORDER BY display_order ASC, id ASC");
$titles = [];
if ($result) {
                                ...
                                <?php foreach ($titles as $row): ?>
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <a href="mst_titles.php?delete_id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('ยืนยันการลบข้อมูลนี้?');">
