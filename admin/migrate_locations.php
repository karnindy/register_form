<?php
// require_once 'includes/auth.php'; // Only admins should run this
...
$db->query("TRUNCATE TABLE mst_provinces");
$db->query("TRUNCATE TABLE mst_districts");
$db->query("TRUNCATE TABLE mst_sub_districts");

// 2. Import Provinces
echo "Importing provinces...\n<br>";
$provinces_json = file_get_contents('../provinces.json');
if ($provinces_json) {
    $provinces = json_decode($provinces_json, true)['provinces'] ?? [];
    ...
    foreach ($provinces as $p) {
        $region_id = 0; // Not available in JSON
        $stmt->bind_param("isssi", $p['PROVINCE_ID'], $p['CODE'], $p['PROVINCE_THAI'], $p['PROVINCE_ENGLISH'], $region_id);
        $stmt->execute();
    }
    $stmt->close();
}
...
if ($districts_json) {
    $districts = json_decode($districts_json, true)['districts'] ?? [];
...
if ($sub_districts_json) {
    $sub_districts = json_decode($sub_districts_json, true)['sub_districts'] ?? [];
